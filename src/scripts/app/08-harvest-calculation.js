...
function getFullHarvestRecordsWithinPlantingLock(referenceDay, sourceRecords){
  ...
  for(const record of (Array.isArray(sourceRecords) ? sourceRecords : [])){
    ...
    const harvestTime = startOfLocalDay(harvestDate).getTime();
    if(harvestTime > referenceTime) continue;
    // 収穫記録は新しい順で保持しているため、10日より前へ到達したら終了できる。
    if(harvestTime < earliestTime) break;
    if(record?.type === "partialHarvest") continue;
...
function getHarvestAvailabilityPlantingInfo(
  recentHarvestRecords,
  referenceDay,
  sourcePlantingEvents,
  lifecycleSourceRecords
){
  const cycleHarvestInfoByPallet = getLatestFullHarvestInfoByPallet(
    recentHarvestRecords,
    referenceDay
  );
  const recentLifecycleHarvestInfoByPallet = getLatestFullHarvestInfoByPallet(
    getFullHarvestRecordsWithinPlantingLock(referenceDay, lifecycleSourceRecords),
    referenceDay
  );
  const recentHarvestIds = new Set(
    (Array.isArray(recentHarvestRecords) ? recentHarvestRecords : [])
      .map(record => getSafePositiveRecordId(record?.id))
      .filter(recordId => recordId !== null)
  );
  const recentHarvestDays = (Array.isArray(recentHarvestRecords) ? recentHarvestRecords : [])
    .map(record => parseDateOnlyString(record?.date))
    .filter(Boolean)
    .map(date => startOfLocalDay(date).getTime());
  const lockStartTime = addDays(
    referenceDay,
    -(HARVEST_SELECTION_PLANTING_LOCK_DAYS - 1)
  ).getTime();
  const earliestRelevantTime = recentHarvestDays.length
    ? Math.min(lockStartTime, ...recentHarvestDays)
    : lockStartTime;
  const referenceTime = referenceDay.getTime();
  const cycleReplantingInfoByPallet = new Map();
  const recentPlantingLockInfoByPallet = new Map();
  const events = Array.isArray(sourcePlantingEvents) ? sourcePlantingEvents : [];

  // 苗植え記録は古い順で保持している。末尾から必要期間だけを確認する。
  for(let eventIndex = events.length - 1; eventIndex >= 0; eventIndex--){
    ...
    const plantingDay = startOfLocalDay(plantingDate);
    const plantingTime = plantingDay.getTime();
    if(plantingTime > referenceTime) continue;
    if(plantingTime < earliestRelevantTime) break;

    const eventId = Number(event?.eventId) || 0;
    const ageDays = getLocalDayDiff(plantingDay, referenceDay);
    const isWithinLockPeriod = ageDays >= 0
      && ageDays < HARVEST_SELECTION_PLANTING_LOCK_DAYS;

    (Array.isArray(event?.sourceAllocations) ? event.sourceAllocations : []).forEach(allocation => {
      const harvestRecordId = Number(allocation?.harvestRecordId);
      (Array.isArray(allocation?.palletKeys) ? allocation.palletKeys : []).forEach(key => {
        const cycleHarvestInfo = cycleHarvestInfoByPallet.get(key);
        if(recentHarvestIds.has(harvestRecordId)
          && cycleHarvestInfo?.recordId === harvestRecordId
          && plantingTime >= cycleHarvestInfo.harvestDay.getTime()
          && !cycleReplantingInfoByPallet.has(key)){
          cycleReplantingInfoByPallet.set(key, {
            eventId,
            harvestRecordId,
            plantingDay,
            ageDays,
        ...
        if(!isWithinLockPeriod || recentPlantingLockInfoByPallet.has(key)) return;
        const recentLifecycleHarvestInfo = recentLifecycleHarvestInfoByPallet.get(key);
        if(recentLifecycleHarvestInfo
          && (recentLifecycleHarvestInfo.harvestDay.getTime() > plantingTime
            || (recentLifecycleHarvestInfo.harvestDay.getTime() === plantingTime
              && recentLifecycleHarvestInfo.recordId !== harvestRecordId))){
          return;
        }
        recentPlantingLockInfoByPallet.set(key, {
          eventId,
          harvestRecordId,
          plantingDay,
          ageDays,
          remainingLockDays:HARVEST_SELECTION_PLANTING_LOCK_DAYS - ageDays,
  ...
  return {
    cycleReplantingInfoByPallet,
    recentPlantingLockInfoByPallet
  };
}
...
function getLatestReplantingInfoByPalletFromRecords(
  ...
  (Array.isArray(sourcePlantingEvents) ? sourcePlantingEvents : []).forEach(event => {
    ...
    if(!plantingDate) return;
    const plantingDay = startOfLocalDay(plantingDate);
    if(plantingDay.getTime() > referenceDay.getTime()) return;
    (Array.isArray(event?.sourceAllocations) ? event.sourceAllocations : []).forEach(allocation => {
      const harvestRecordId = Number(allocation?.harvestRecordId);
      (Array.isArray(allocation?.palletKeys) ? allocation.palletKeys : []).forEach(key => {
...
function getHarvestAvailabilityStateFromRecords(
  ...
  const {
    cycleReplantingInfoByPallet,
    recentPlantingLockInfoByPallet
  } = getHarvestAvailabilityPlantingInfo(
    recentHarvestRecords,
    referenceDay,
    sourcePlantingEvents,
    lifecycleSourceRecords
  );
  const replantingInfoByPallet = new Map(cycleReplantingInfoByPallet);
  const unavailableSet = new Set(cycleRecordedSet);
  const plantingLockedSet = new Set();

  cycleReplantingInfoByPallet.forEach((info, key) => {
    if(info.isSelectionLocked){
      unavailableSet.add(key);
      plantingLockedSet.add(key);
    }else{
  ...
  recentPlantingLockInfoByPallet.forEach((info, key) => {
    unavailableSet.add(key);
    plantingLockedSet.add(key);
    const current = replantingInfoByPallet.get(key);
    if(!current
      || current.plantingDay.getTime() < info.plantingDay.getTime()
      || (current.plantingDay.getTime() === info.plantingDay.getTime()
        && current.eventId < info.eventId)){
      replantingInfoByPallet.set(key, info);
    }
  });
  ...
  const unplantedSet = new Set();
  cycleRecordedSet.forEach(key => {
    if(!cycleReplantingInfoByPallet.has(key)) unplantedSet.add(key);
  });
...
function invalidateHarvestAvailabilityStateCache(){
  harvestAvailabilityStateCache = new WeakMap();
  harvestAvailabilityStateCachePlantingEvents = plantingEvents;
}

function getHarvestAvailabilityState(referenceDate = new Date(), sourceRecords = getActiveHarvestTimelineRecords(records)){
  if(harvestAvailabilityStateCachePlantingEvents !== plantingEvents){
    invalidateHarvestAvailabilityStateCache();
  }
  const referenceDay = referenceDate instanceof Date
    ? startOfLocalDay(referenceDate)
    : startOfLocalDay(parseDateOnlyString(String(referenceDate || "")) || new Date());
  const cacheKey = formatDateOnlyString(referenceDay);
  const cacheableSourceRecords = Array.isArray(sourceRecords) ? sourceRecords : [];
  let sourceCache = harvestAvailabilityStateCache.get(cacheableSourceRecords);
  if(!sourceCache){
    sourceCache = new Map();
    harvestAvailabilityStateCache.set(cacheableSourceRecords, sourceCache);
  }
...
function getRecordTabHarvestAvailabilityState(){
  ...
  return getHarvestAvailabilityState(referenceDate, sourceRecords);
}
...
function isBedFullyFilledInCurrentBuilding(
  bed,
