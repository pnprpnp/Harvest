...
const GOOGLE_SHEET_ACCEPTED_BATCHES_KEY = "harvestForecastGoogleSheetAcceptedBatches_v1";
const GOOGLE_SHEET_ACCEPTED_BATCH_PAYLOADS_KEY = "harvestForecastGoogleSheetAcceptedBatchPayloads_v1";
...
const WORKER_GOOGLE_SHEET_ACCEPTED_BATCHES_KEY = "harvestnaviWorkerGoogleSheetAcceptedBatches_v1";
const WORKER_GOOGLE_SHEET_ACCEPTED_BATCH_PAYLOADS_KEY = "harvestnaviWorkerGoogleSheetAcceptedBatchPayloads_v1";
...
const RECORD_RELAY_TIMEOUT_MS = 5000;
const GOOGLE_SHEET_SEND_RETRY_DELAYS_MS = Object.freeze([3000, 10000, 30000]);
const GOOGLE_SHEET_INBOX_STATUS_DELAYS_MS = Object.freeze([5000, 15000, 30000, 60000]);
...
const PLANTING_RESUME_RECORD_LIMIT = 3;
...
const RECORD_HISTORY_MONTH_BATCH_SIZE = 3;
const DASHBOARD_RECORD_FILTER_DELAY_MS = 180;
const HARVEST_STATE_SAVE_DELAY_MS = 250;
const WELCOME_MINIMUM_VISIBLE_MS = 450;
const WELCOME_FADE_MS = 200;
...
function getActiveGoogleSheetAcceptedBatchesStorageKey(){ return getRoleScopedStorageKey(GOOGLE_SHEET_ACCEPTED_BATCHES_KEY, WORKER_GOOGLE_SHEET_ACCEPTED_BATCHES_KEY); }
function getActiveGoogleSheetAcceptedBatchPayloadsStorageKey(){
  return getRoleScopedStorageKey(
    GOOGLE_SHEET_ACCEPTED_BATCH_PAYLOADS_KEY,
    WORKER_GOOGLE_SHEET_ACCEPTED_BATCH_PAYLOADS_KEY
  );
}
...
let harvestProgressEntryDetailGroupKey = "";
let harvestProgressEntryEditState = null;
...
let dashboardSeedlingStatusSelectedDateIndex = null;
...
let harvestLossEstimatedRecordCachePlantingEvents = plantingEvents;
let harvestLossEstimatedRecordCacheSettings = settings;
...
let googleSheetInboxSupportState = "unknown";
const googleSheetAcceptedBatchQueue = new Map();
let googleSheetAcceptedBatchStorageMode = "split";
let googleSheetAcceptedBatchCheckRunning = false;
let googleSheetAcceptedBatchCheckTimer = null;
let googleSheetAcceptedBatchCheckTimerDueAt = 0;
...
function openDashboardSeedlingStatusInfoWindow(){
  const modal = document.getElementById("dashboardSeedlingStatusInfoModal");
  const closeButton = document.getElementById("dashboardSeedlingStatusInfoWindowClose");
  if(!modal) return;
  dashboardSeedlingStatusInfoReturnFocus = document.activeElement;
  showPageBlockingUi(modal);
  requestAnimationFrame(() => closeButton?.focus());
}

function closeDashboardSeedlingStatusInfoWindow(){
  const modal = document.getElementById("dashboardSeedlingStatusInfoModal");
  hidePageBlockingUi(modal);
...
function renderHarvestSelectionMapsForActiveTab(options = {}){
  if(activeAppTab === "forecast"){
    ...
    if(options.preferIncremental && updateRecordBeds(options)) return null;
    drawRecordBeds();
  }
  return null;
}
...
function ensureHarvestProgressEntriesForEditing(){
  const state = normalizeHarvestProgressState(harvestProgressState);
  if(!state) return null;
  if(!state.entries.length
    && state.appliedSelectedBeds.length
    && state.appliedActualCases !== null){
    const legacyEntry = normalizeHarvestProgressEntry({
      type:"regular",
      bedKeys:state.appliedSelectedBeds,
      cases:state.appliedActualCases,
      recordedAt:Date.now()
    });
    if(legacyEntry) state.entries = [legacyEntry];
  }
  harvestProgressState = state;
  return state;
}

function getHarvestProgressEntryDisplayGroups(state = harvestProgressState){
  const normalized = normalizeHarvestProgressState(state);
  if(!normalized) return [];
...
function getHarvestProgressEntryDisplayLines(state = harvestProgressState){
  return getHarvestProgressEntryDisplayGroups(state).map(group => group.label);
}

function renderHarvestProgressEntryResult(result, state, model){
  if(!result) return;
  const groups = getHarvestProgressEntryDisplayGroups(state);
  result.replaceChildren();
  if(groups.length){
    const heading = document.createElement("div");
    heading.className = "harvestProgressResultHeading";
    heading.textContent = "入力済み";
    result.appendChild(heading);
    result.appendChild(document.createTextNode("\n"));
    const list = document.createElement("div");
    list.className = "harvestProgressEntryList";
    groups.forEach(group => {
      ...
      button.onclick = () => openHarvestProgressEntryDetails(group.key);
      list.appendChild(button);
      if(group !== groups[groups.length - 1]) list.appendChild(document.createTextNode("\n"));
    });
    result.appendChild(list);
  }
  if(model.text){
    if(groups.length) result.appendChild(document.createTextNode("\n\n"));
    const message = document.createElement("div");
    message.className = "harvestProgressResultMessage";
    message.textContent = model.text;
    result.appendChild(message);
  }
}

function renderHarvestProgressEntryDetails(){
  const panel = document.getElementById("harvestProgressEntryDetails");
  const rows = document.getElementById("harvestProgressEntryDetailsRows");
  const location = document.getElementById("harvestProgressEntryDetailsLocation");
  if(!panel || !rows || !location) return;
  const group = getHarvestProgressEntryDisplayGroups()
    .find(item => item.key === harvestProgressEntryDetailGroupKey);
  if(!group){
    panel.hidden = true;
  ...
  group.entries.forEach((entry, index) => {
    ...
    row.className = "harvestProgressEntryDetailRow";
    const summary = document.createElement("div");
    const type = document.createElement("div");
    type.className = "harvestProgressEntryDetailType";
    const sameTypeCount = group.entries.filter(item => item.type === entry.type).length;
    const sameTypeNumber = group.entries
      .slice(0, index + 1)
      .filter(item => item.type === entry.type).length;
    type.textContent = (entry.type === "partial" ? "部分収穫" : "通常収穫")
      + (sameTypeCount > 1 ? ` ${sameTypeNumber}` : "");
    ...
    edit.textContent = "修正";
    edit.onclick = () => startHarvestProgressEntryEdit(entry.entryIndex);
...
function openHarvestProgressEntryDetails(groupKey){
  const state = ensureHarvestProgressEntriesForEditing();
  if(!state) return false;
  harvestProgressEntryDetailGroupKey = String(groupKey || "");
  renderHarvestProgressEntryDetails();
  document.getElementById("harvestProgressEntryDetailsClose")?.focus();
  return !document.getElementById("harvestProgressEntryDetails")?.hidden;
}

function closeHarvestProgressEntryDetails(options = {}){
  const panel = document.getElementById("harvestProgressEntryDetails");
  if(panel) panel.hidden = true;
  const groupKey = harvestProgressEntryDetailGroupKey;
  harvestProgressEntryDetailGroupKey = "";
  if(options.restoreFocus !== false){
    const button = [...document.querySelectorAll("#harvestProgressResult .harvestProgressEntryLine")]
      .find(item => item.getAttribute("aria-label")?.startsWith(
        getHarvestProgressEntryDisplayGroups().find(group => group.key === groupKey)?.label || "\u0000"
      ));
    button?.focus();
  }
  return true;
...
function getHarvestProgressEntryEditModel(){
  const source = harvestProgressEntryEditState;
  if(!source || typeof source !== "object") return null;
  const state = normalizeHarvestProgressState(harvestProgressState);
  const entryIndex = Number(source.entryIndex);
  const entry = Number.isInteger(entryIndex) ? state?.entries?.[entryIndex] : null;
  if(!state || !entry || entry.type !== source.type) return null;
  return {
    entryIndex,
    partialEntryIndex:Number(source.partialEntryIndex),
    type:entry.type,
    bedKeys:[...new Set((source.bedKeys || []).map(normalizeHarvestProgressBedKey).filter(Boolean))],
    cases:String(source.cases ?? "").trim()
  };
}

function startHarvestProgressEntryEdit(entryIndex){
  const state = ensureHarvestProgressEntriesForEditing();
  ...
  const partialEntryIndex = entry.type === "partial"
    ? state.entries.slice(0, safeIndex).filter(item => item.type === "partial").length
    : -1;
  harvestProgressEntryEditState = {
    entryIndex:safeIndex,
    partialEntryIndex,
    type:entry.type,
    bedKeys:[...entry.bedKeys],
    cases:formatHarvestProgressCases(entry.cases)
  };
  closeHarvestProgressEntryDetails({ restoreFocus:false });
  const firstBuilding = Number(String(entry.bedKeys[0] || "").split("-")[0]);
  if(BUILDINGS.includes(firstBuilding)) harvestProgressBuilding = firstBuilding;
  updateHarvestProgressUi();
  document.getElementById("harvestProgressCasesInput")?.focus();
  return true;
}

function cancelHarvestProgressEntryEdit(){
  if(!harvestProgressEntryEditState) return false;
  harvestProgressEntryEditState = null;
  updateHarvestProgressUi();
  return true;
}

function getHarvestProgressEntryAssignedBedSet(type, excludedEntryIndex){
  const state = normalizeHarvestProgressState(harvestProgressState);
  const assigned = new Set();
  state?.entries?.forEach((entry, index) => {
    if(index === excludedEntryIndex || entry.type !== type) return;
    entry.bedKeys.forEach(key => assigned.add(key));
  });
  return assigned;
}

function toggleHarvestProgressEntryEditBed(building, bed){
  const edit = getHarvestProgressEntryEditModel();
  if(!edit) return false;
  const bedKey = getHarvestProgressBedKey(building, bed);
  const selected = new Set(edit.bedKeys);
  if(selected.has(bedKey)){
    selected.delete(bedKey);
  }else{
    const assigned = getHarvestProgressEntryAssignedBedSet(edit.type, edit.entryIndex);
    if(assigned.has(bedKey)){
      showToast(edit.type === "partial"
        ? "このベッドは別の部分収穫へ入力済みです"
        : "このベッドは別の途中経過へ入力済みです");
      return false;
    }
    if(edit.type === "regular"
      && isReverseHarvestProgressState()
      && !hasHarvestProgressPlannedPalletInBed(harvestProgressState, building, bed)){
      showToast("逆算時に選択した場所を含むベッドだけ指定できます");
      return false;
    }
    selected.add(bedKey);
  }
  harvestProgressEntryEditState = { ...edit, bedKeys:[...selected] };
  updateHarvestProgressUi();
  return true;
}

function rebuildHarvestProgressAfterRegularEntryChange(){
  const state = normalizeHarvestProgressState(harvestProgressState);
  ...
  state.appliedActualCases = regularEntries.length ? Math.min(999999, actualCases) : null;
  state.actualCasesInput = "";
  harvestProgressState = state;

  const completedKeys = getHarvestProgressKeysForBeds(completedBeds, state);
  const completedSet = new Set(completedKeys);
  const remainingNeedHeads = Math.max(0, getHarvestCasePlan().regularCases - actualCases) * CASE_SIZE;
  let remainingKeys = [];
  let selection = null;
  ...
  if(activeAppTab !== "forecast") updateHarvestProgressUi();
  return true;
}

function saveHarvestProgressEntryEdit(){
  const edit = getHarvestProgressEntryEditModel();
  const state = normalizeHarvestProgressState(harvestProgressState);
  if(!edit || !state){
  ...
  if(!edit.bedKeys.length){
    showToast("収穫したベッドを1つ以上選択してください");
    return false;
  }
  const assignedBeds = getHarvestProgressEntryAssignedBedSet(edit.type, edit.entryIndex);
  if(edit.bedKeys.some(key => assignedBeds.has(key))){
    showToast("同じ種類の別の入力で使用中のベッドがあります");
    return false;
  }
  const cases = edit.type === "partial"
    ? getStrictIntegerInRange(edit.cases, 1, RECORD_MAX_CASES)
    : Number(edit.cases);
  if(cases === null || !Number.isFinite(cases) || cases <= 0 || cases > 999999){
    showToast(edit.type === "partial"
      ? "部分収穫のケース数を1以上の整数で入力してください"
      : "収穫したケース数を1以上で入力してください");
    document.getElementById("harvestProgressCasesInput")?.focus();
    return false;
  }
  ...
  if(edit.type === "partial"){
    ...
    entries[edit.partialEntryIndex] = { bedKeys:[...edit.bedKeys], cases:String(cases) };
    ...
    const predictionUpdate = recalculateHarvestPredictionAfterPartialHarvest([getHarvestTargetDateString()]);
    ...
    const replacement = normalizeHarvestProgressEntry({
      type:"regular",
      bedKeys:edit.bedKeys,
      cases,
      recordedAt:state.entries[edit.entryIndex].recordedAt || Date.now()
    });
    if(!replacement) return false;
    state.entries[edit.entryIndex] = replacement;
    harvestProgressState = state;
    harvestProgressEntryEditState = null;
    rebuildHarvestProgressAfterRegularEntryChange();
  }
  scheduleHarvestStateSave();
  showToast("途中経過を修正しました");
  return true;
}

function deleteHarvestProgressEntryEdit(){
  const edit = getHarvestProgressEntryEditModel();
  const state = normalizeHarvestProgressState(harvestProgressState);
  ...
  if(edit.type === "partial"){
    ...
    if(!Number.isInteger(edit.partialEntryIndex)
      || edit.partialEntryIndex < 0
      || edit.partialEntryIndex >= draft.entries.length) return false;
    const entries = draft.entries
      .filter((entry, index) => index !== edit.partialEntryIndex)
      .map(entry => ({ ...entry, bedKeys:[...entry.bedKeys] }));
    recordPartialHarvestDraft = normalizeRecordPartialHarvestDraft({
      entries,
      bedKeys:[],
      cases:"",
    ...
    state.entries = state.entries.filter((entry, index) => index !== edit.entryIndex);
    harvestProgressState = state;
    harvestProgressEntryEditState = null;
    rebuildHarvestProgressAfterRegularEntryChange();
  }
  scheduleHarvestStateSave();
  showToast("途中経過の入力を削除しました");
  return true;
}
...
function renderHarvestProgressBeds(){
  ...
  const entryEdit = getHarvestProgressEntryEditModel();
  const isEntryEditMode = !!entryEdit;
  ...
  if(harvestProgressPartialSelectionMode || entryEdit?.type === "partial"){
    partialDraft.committedEntries.forEach((entry, index) => {
      const excludedIndex = entryEdit?.type === "partial"
        ? entryEdit.partialEntryIndex
        : partialDraft.editingEntryIndex;
      if(index === excludedIndex) return;
      entry.bedKeys.forEach(key => assignedPartialEntryByBed.set(key, entry));
    });
  }
  const assignedRegularBedSet = entryEdit?.type === "regular"
    ? getHarvestProgressEntryAssignedBedSet("regular", entryEdit.entryIndex)
    : new Set();
  ...
  bedMap.forEach(bedName => {
    ...
    const isRegularAssigned = assignedRegularBedSet.has(bedKey);
    const isApplied = !harvestProgressPartialSelectionMode && !isEntryEditMode
    ...
    const canSelectBed = entryEdit?.type === "partial"
      ? (!isPartialAssigned || isSelected)
      : (entryEdit?.type === "regular"
          ? ((!isRegularAssigned || isSelected)
    ...
    button.className = "harvestProgressBed"
      ...
      + ((harvestProgressPartialSelectionMode || entryEdit?.type === "partial") ? " is-partial-selectable" : "");
    ...
    button.setAttribute(
      ...
      entryEdit
        ? `${harvestProgressBuilding}号棟 ${bedName}ベッド。${isSelected
            ? "修正する場所として選択中。タップで切り替え"
            : ((isPartialAssigned || isRegularAssigned)
                ? "別の入力で使用中"
                : "タップして修正する場所に選択")}`
        : harvestProgressPartialSelectionMode
        ? `${harvestProgressBuilding}号棟 ${bedName}ベッド。${isSelected
    ...
    button.onclick = () => entryEdit
      ? toggleHarvestProgressEntryEditBed(harvestProgressBuilding, bedName)
      : (harvestProgressPartialSelectionMode
...
function updateHarvestProgressUi(options = {}){
  ...
  const entryEdit = getHarvestProgressEntryEditModel();
  const isEntryEditMode = !!entryEdit;
  if(harvestProgressEntryEditState && !entryEdit) harvestProgressEntryEditState = null;
  const input = document.getElementById("harvestProgressCasesInput");
  const inputMode = isEntryEditMode
    ? `entry-${entryEdit.entryIndex}`
    : (isPartialMode ? "partial" : "regular");
  if(input){
    const inputModeChanged = input.dataset.harvestProgressInputMode !== inputMode;
    if(document.activeElement !== input || inputModeChanged){
      input.value = isEntryEditMode
        ? entryEdit.cases
        : (isPartialMode ? partialDraft.cases : (state?.actualCasesInput || ""));
    }
    input.min = (isPartialMode || isEntryEditMode) ? "1" : "0";
    input.step = entryEdit?.type === "regular" ? "0.1" : "1";
    input.dataset.harvestProgressInputMode = inputMode;
  }
  const casesLabel = document.getElementById("harvestProgressCasesLabel");
  if(casesLabel){
    casesLabel.textContent = isEntryEditMode
      ? `${entryEdit.type === "partial" ? "部分収穫" : "通常収穫"}のケース数`
      : isPartialMode
  ...
  if(selectionText){
    ...
    selectionText.textContent = isEntryEditMode
      ? (entryEdit.bedKeys.length
          ? `修正後: ${formatHarvestProgressBedSelection(entryEdit.bedKeys)}`
          : "修正後の収穫場所を選択してください")
      : isPartialMode
      ? (partialDraft.bedKeys.length > 1
  ...
  if(result){
    const model = (isPartialMode || isEntryEditMode)
      ? { text:"", className:"" }
      : getHarvestProgressResultModel(options.currentHarvestTotal);
    if(!isEntryEditMode && !isPartialMode) renderHarvestProgressEntryResult(result, state, model);
    else result.replaceChildren();
    result.hidden = isPartialMode || isEntryEditMode || !result.textContent;
  ...
  actions?.classList.toggle("is-entry-edit-mode", isEntryEditMode);
  const resetButton = document.getElementById("harvestProgressResetBtn");
  if(resetButton){
    resetButton.hidden = isEntryEditMode ? false : (isPartialMode ? false : !state);
    resetButton.setAttribute("data-ui-click", isEntryEditMode
      ? "cancelHarvestProgressEntryEdit"
      : "resetHarvestProgress");
    resetButton.textContent = isEntryEditMode
      ? "キャンセル"
      : (isPartialMode && partialDraft.committedEntries.length ? "完了" : (isPartialMode ? "キャンセル" : "取り消す"));
  }
  const applyButton = document.getElementById("harvestProgressApplyBtn");
  if(applyButton){
    applyButton.setAttribute("data-ui-click", isEntryEditMode
      ? "saveHarvestProgressEntryEdit"
      : "recalculateFromHarvestProgress");
    applyButton.textContent = isEntryEditMode ? "保存" : (isPartialMode ? "決定" : getHarvestProgressApplyLabel(state));
    applyButton.disabled = isEntryEditMode
      ? (!entryEdit.bedKeys.length || !String(entryEdit.cases || "").trim())
      : (isPartialMode && !partialDraft.activeIsValid);
  }
  const deleteButton = document.getElementById("harvestProgressEntryDeleteBtn");
  if(deleteButton) deleteButton.hidden = !isEntryEditMode;
  ...
  if(partialButton){
    partialButton.hidden = isPartialMode || isEntryEditMode;
  ...
  const modal = document.getElementById("harvestProgressModal");
  if(modal?.classList.contains("show") && !options.skipBeds) renderHarvestProgressBeds();
}
...
function startHarvestProgressPartialHarvestSelection(){
  ...
  if(harvestProgressEntryEditState) return;
...
function handleHarvestProgressCasesInput(){
  ...
  if(entryEdit){
    harvestProgressEntryEditState = {
      ...entryEdit,
      cases:document.getElementById("harvestProgressCasesInput")?.value || ""
    };
    updateHarvestProgressUi({ skipBeds:true });
    scheduleHarvestStateSave();
    return;
  }
  if(harvestProgressPartialSelectionMode){
