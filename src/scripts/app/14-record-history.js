// ===== 記録一覧：編集・履歴・削除 =====
let recordHistoryVisibleMonthLimit = RECORD_HISTORY_MONTH_BATCH_SIZE;
let recordHistoryOpenMonthKey = null;
...
function invalidateRecordHistoryCache(){
  ...
  recordHistoryRenderedCache = null;
}

function getRecordHistoryMonthKey(value){
  const date = parseDateOnlyString(String(value || "").trim());
  if(!date) return "undated";
  return `${date.getFullYear()}-${String(date.getMonth() + 1).padStart(2, "0")}`;
}

function formatRecordHistoryMonthLabel(monthKey){
...
function getRecordHistoryCache(){
  ...
  historyItems.forEach(item => {
    ...
    const monthKey = getRecordHistoryMonthKey(dateKey);
...
function splitRecordHistoryDayItems(items){
  const group = { harvest:[], planting:[], partial:[] };
...
function renderRecordHistoryItemHtml(item, cache){
  const issue = cache.consistencyAudit.issueByKey.get(
    getRecordConsistencyIssueKey(item.kind, item.id)
  ) || null;
  return item.kind === "planting"
    ? renderPlantingEventItemHtml(item.value, issue)
    : renderRecordItemHtml(item.value, cache.harvestCaseTotalsByDate, issue);
}

function renderRecordHistoryDateBodyHtml(dateKey, cache = getRecordHistoryCache()){
  const group = splitRecordHistoryDayItems(cache.itemsByDate.get(dateKey) || []);
  const harvestHtml = group.harvest.map(item => renderRecordHistoryItemHtml(item, cache)).join("");
  const plantingHtml = group.planting.map(item => renderRecordHistoryItemHtml(item, cache)).join("");
  const partialHtml = group.partial.length
    ? renderPartialHarvestDaySummaryHtml(group.partial, cache.consistencyAudit)
    : "";
  const primaryHtml = harvestHtml || plantingHtml
    ...
    : "";
  return `${primaryHtml}${partialHtml ? `<div class="recordDateColumn recordDatePartialColumn">${partialHtml}</div>` : ""}`;
}

function renderRecordHistoryDateGroupHtml(dateKey, items, cache, isOpen){
  const group = splitRecordHistoryDayItems(items);
  const caseTotals = getHarvestCaseTotalsForDate(dateKey, records, cache.harvestCaseTotalsByDate);
  const plantingMetrics = getPlantingEventGroupListMetrics(
    group.planting.map(item => item.value)
  );
  const typeBadges = [
    group.harvest.length
      ? `<span class="recordDateGroupType is-harvest">収穫 ${escapeHtml(String(caseTotals.totalCases))}ケース</span>`
      : "",
    group.planting.length
      ? `<span class="recordDateGroupType is-planting">苗植え ${escapeHtml(plantingMetrics.seedlingTrayText)}</span>`
      : "",
    group.partial.length
      ? `<span class="recordDateGroupType is-partial">部分</span>`
      : ""
  ].join("");
  const bodyHtml = isOpen ? renderRecordHistoryDateBodyHtml(dateKey, cache) : "";
  return `<section class="recordDateGroup${isOpen ? " is-open" : ""}" data-record-history-date="${escapeHtml(dateKey)}">
    <button type="button" class="recordDateGroupHeader" aria-expanded="${isOpen ? "true" : "false"}"
      data-ui-click="toggleRecordHistoryDateGroup" data-ui-arg="${escapeHtml(dateKey)}">
      <span class="recordDateGroupTitle">${escapeHtml(formatRecordHistoryDateLabel(dateKey))}</span>
      <span class="recordDateGroupTypes" aria-label="この日の記録">${typeBadges}</span>
      <span class="recordDateGroupChevron" aria-hidden="true">›</span>
    </button>
    <div class="recordDateGroupItems" data-record-history-date-body data-rendered="${isOpen ? "true" : "false"}"${isOpen ? "" : " hidden"}>
      ${bodyHtml}
    </div>
  </section>`;
}

function renderRecordHistoryMonthBodyHtml(monthKey, cache = getRecordHistoryCache()){
  const dateGroups = new Map();
  (cache.itemsByMonth.get(monthKey) || []).forEach(item => {
    const dateKey = String(item.date || "");
    if(!dateGroups.has(dateKey)) dateGroups.set(dateKey, []);
    dateGroups.get(dateKey).push(item);
  });
  return [...dateGroups.entries()].map(([dateKey, items]) => (
    renderRecordHistoryDateGroupHtml(
      dateKey,
      items,
      cache,
      recordHistoryOpenDateKeys.has(dateKey)
    )
  )).join("");
}

function renderRecordHistoryMonthGroupHtml(monthKey, cache, isOpen = false){
  const bodyHtml = isOpen ? renderRecordHistoryMonthBodyHtml(monthKey, cache) : "";
  return `<section class="recordMonthGroup${isOpen ? " is-open" : ""}" data-record-history-month="${escapeHtml(monthKey)}">
    <button type="button" class="recordMonthGroupHeader" aria-expanded="${isOpen ? "true" : "false"}"
      data-ui-click="toggleRecordHistoryMonth" data-ui-arg="${escapeHtml(monthKey)}">
      <span class="recordMonthGroupTitle">${escapeHtml(formatRecordHistoryMonthLabel(monthKey))}</span>
      <span class="recordMonthGroupChevron" aria-hidden="true">›</span>
    </button>
    <div class="recordMonthGroupBody" data-record-history-month-body data-rendered="${isOpen ? "true" : "false"}"${isOpen ? "" : " hidden"}>
      ${bodyHtml}
    </div>
  </section>`;
}

function renderRecordHistoryMoreHtml(hiddenMonthCount){
  if(hiddenMonthCount <= 0) return "";
  return `<div class="recordHistoryMore">
    <button type="button" class="recordHistoryMoreBtn" data-ui-click="showMoreRecordHistory">さらに過去の月を表示（残り ${hiddenMonthCount}か月）</button>
  </div>`;
}

function findRecordHistoryElement(attributeName, value){
  return [...document.querySelectorAll(`#recordList [${attributeName}]`)]
    .find(element => element.getAttribute(attributeName) === String(value || "")) || null;
}

function setRecordHistoryMonthExpanded(group, expanded){
  if(!group) return;
  group.classList.toggle("is-open", expanded);
  group.querySelector(":scope > .recordMonthGroupHeader")?.setAttribute("aria-expanded", String(expanded));
  const body = group.querySelector(":scope > .recordMonthGroupBody");
  if(body) body.hidden = !expanded;
}

function setRecordHistoryDateExpanded(group, expanded){
  if(!group) return;
  const dateKey = String(group.dataset.recordHistoryDate || "");
  group.classList.toggle("is-open", expanded);
  group.querySelector(":scope > .recordDateGroupHeader")?.setAttribute("aria-expanded", String(expanded));
  const body = group.querySelector(":scope > .recordDateGroupItems");
  if(body) body.hidden = !expanded;
  if(expanded) recordHistoryOpenDateKeys.add(dateKey);
  else recordHistoryOpenDateKeys.delete(dateKey);
}

function scrollRecordHistoryGroupHeaderIntoView(group, headerSelector){
  const header = group?.querySelector(headerSelector);
  if(!header) return;
  const top = header.getBoundingClientRect().top + window.pageYOffset - getAppTopChromeOffset() - 8;
  window.scrollTo({ top:Math.max(0, top), behavior:"auto" });
}

function toggleRecordHistoryMonth(monthKey){
  const target = findRecordHistoryElement("data-record-history-month", monthKey);
  if(!target) return;
  const wasOpen = target.classList.contains("is-open");
  document.querySelectorAll("#recordList .recordMonthGroup.is-open").forEach(group => (
    setRecordHistoryMonthExpanded(group, false)
  ));
  if(wasOpen){
    recordHistoryOpenMonthKey = "";
    scrollRecordHistoryGroupHeaderIntoView(target, ":scope > .recordMonthGroupHeader");
    return;
  }

  const body = target.querySelector(":scope > .recordMonthGroupBody");
  if(body?.dataset.rendered !== "true"){
    body.innerHTML = renderRecordHistoryMonthBodyHtml(monthKey);
    body.dataset.rendered = "true";
  }
  recordHistoryOpenMonthKey = String(monthKey || "");
  setRecordHistoryMonthExpanded(target, true);
  scrollRecordHistoryGroupHeaderIntoView(target, ":scope > .recordMonthGroupHeader");
}

function scheduleRecordHistoryDateGroupOpen(dateKey){
  const group = findRecordHistoryElement("data-record-history-date", dateKey);
  if(!group) return;
  const scheduleId = ++recordHistoryRenderScheduleId;
  recordHistoryRenderPending = true;
  group.setAttribute("aria-busy", "true");
  document.getElementById("recordHistoryCard")?.setAttribute("aria-busy", "true");
  setMainTabLoadingState("record", true, {
    immediate:true,
    label:formatRecordHistoryDateLabel(dateKey) + "の記録"
  });
  runAfterUiSettles(() => {
    ...
    try{
      const currentGroup = findRecordHistoryElement("data-record-history-date", dateKey);
      const body = currentGroup?.querySelector(":scope > .recordDateGroupItems");
      if(!currentGroup || !body) return;
      body.innerHTML = renderRecordHistoryDateBodyHtml(dateKey);
      body.dataset.rendered = "true";
      setRecordHistoryDateExpanded(currentGroup, true);
      requestAnimationFrame(() => (
        scrollRecordHistoryGroupHeaderIntoView(currentGroup, ":scope > .recordDateGroupHeader")
      ));
    }catch(error){
...
function toggleRecordHistoryDateGroup(dateKey){
  const group = findRecordHistoryElement("data-record-history-date", dateKey);
  if(!group) return;
  if(group.classList.contains("is-open")){
    setRecordHistoryDateExpanded(group, false);
    scrollRecordHistoryGroupHeaderIntoView(group, ":scope > .recordDateGroupHeader");
    return;
  }
  const body = group.querySelector(":scope > .recordDateGroupItems");
  if(body?.dataset.rendered === "true"){
    setRecordHistoryDateExpanded(group, true);
    scrollRecordHistoryGroupHeaderIntoView(group, ":scope > .recordDateGroupHeader");
    return;
  }
  scheduleRecordHistoryDateGroupOpen(dateKey);
}

function showMoreRecordHistory(){
  const box = document.getElementById("recordList");
  const cache = getRecordHistoryCache();
  const previousLimit = Math.min(recordHistoryVisibleMonthLimit, cache.monthKeys.length);
  recordHistoryVisibleMonthLimit = Math.min(
    cache.monthKeys.length,
    previousLimit + RECORD_HISTORY_MONTH_BATCH_SIZE
  );
  if(!box || recordHistoryRenderedCache !== cache){
    renderRecordList();
    return;
  }

  const more = box.querySelector(".recordHistoryMore");
  const nextMonthKeys = cache.monthKeys.slice(previousLimit, recordHistoryVisibleMonthLimit);
  if(more && nextMonthKeys.length){
    more.insertAdjacentHTML(
      "beforebegin",
      nextMonthKeys.map(monthKey => renderRecordHistoryMonthGroupHtml(monthKey, cache, false)).join("")
    );
  }
  const hiddenMonthCount = Math.max(0, cache.monthKeys.length - recordHistoryVisibleMonthLimit);
  if(more){
    if(hiddenMonthCount){
      more.querySelector(".recordHistoryMoreBtn").textContent = `さらに過去の月を表示（残り ${hiddenMonthCount}か月）`;
    }else{
      more.remove();
    }
  }
...
function renderRecordList(options = {}){
  const box = document.getElementById("recordList");
  updateTodayHarvestRecordedStatus();
  renderDeletedRecordList();
  const cache = getRecordHistoryCache();
  const { historyItems, itemsByDate, monthKeys, consistencyAudit } = cache;
  renderRecordConsistencyAlert(consistencyAudit);
  if(!historyItems.length){
    box.textContent = "まだ記録がありません。";
    recordHistoryRenderedCache = cache;
    return;
  }

  recordHistoryOpenDateKeys = new Set(
    [...recordHistoryOpenDateKeys].filter(dateKey => itemsByDate.has(dateKey))
  );
  const requestedDate = String(options.preferredDate || "");
  const selectedDate = String(document.getElementById("recordDateInput")?.value || "");
  const hasRequestedDate = itemsByDate.has(requestedDate);
  const initialDate = itemsByDate.has(selectedDate)
    ? selectedDate
    : String(historyItems[0]?.date || "");
  const openStateNeedsDefault = recordHistoryOpenMonthKey === null
    || (recordHistoryOpenMonthKey && !monthKeys.includes(recordHistoryOpenMonthKey));
  if(hasRequestedDate){
    recordHistoryOpenMonthKey = getRecordHistoryMonthKey(requestedDate);
    recordHistoryOpenDateKeys.add(requestedDate);
  }else if(openStateNeedsDefault){
    recordHistoryOpenMonthKey = getRecordHistoryMonthKey(initialDate);
    if(initialDate) recordHistoryOpenDateKeys.add(initialDate);
  }
  const openMonthIndex = monthKeys.indexOf(recordHistoryOpenMonthKey);
  if(openMonthIndex >= 0){
    recordHistoryVisibleMonthLimit = Math.max(recordHistoryVisibleMonthLimit, openMonthIndex + 1);
  }
  recordHistoryVisibleMonthLimit = Math.max(
    RECORD_HISTORY_MONTH_BATCH_SIZE,
    Math.min(recordHistoryVisibleMonthLimit, monthKeys.length)
  );
  const visibleMonthKeys = monthKeys.slice(0, recordHistoryVisibleMonthLimit);
  const hiddenMonthCount = Math.max(0, monthKeys.length - visibleMonthKeys.length);
  box.innerHTML = visibleMonthKeys.map(monthKey => (
    renderRecordHistoryMonthGroupHtml(monthKey, cache, monthKey === recordHistoryOpenMonthKey)
  )).join("") + renderRecordHistoryMoreHtml(hiddenMonthCount);
  recordHistoryRenderedCache = cache;
}
