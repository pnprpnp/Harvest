...
function hasPendingGoogleSheetRecordChange(record, status = loadGoogleSheetSyncStatus()){
  return ["edited", "failed", "pending", "accepted", "conflict", "remoteDeleted"].includes(
...
function getGoogleSheetUnsentRecords(){
  ...
  return records.filter(record => (
    ...
    && getGoogleSheetRecordSyncState(record, status) !== "accepted"
...
function getGoogleSheetBackgroundRetryDelays(){
  return [...GOOGLE_SHEET_SEND_RETRY_DELAYS_MS];
}

function normalizeGoogleSheetBackgroundRecordJob(value){
  if(!value || typeof value !== "object") return null;
  const recordUuid = normalizeRecordUuid(value.recordUuid);
  const recordId = getSafePositiveRecordId(value.recordId);
  if(!recordUuid && recordId === null) return null;
  const waitForPlantingRecordId = getSafePositiveRecordId(value.waitForPlantingRecordId);
  const retryCount = clampNumber(
    value.retryCount,
    0,
    getGoogleSheetBackgroundRetryDelays().length,
    0
  );
  return {
    recordUuid,
    recordId,
    waitForPlantingRecordId,
    retryCount: Math.trunc(retryCount),
    nextAttemptAt: Math.max(0, Number(value.nextAttemptAt) || 0),
    successMessage: String(value.successMessage || "").slice(0, 500),
    failureMessage: String(value.failureMessage || "記録は端末内に保存されています。スプレッドシートは未送信です").slice(0, 500)
  };
}

function normalizeGoogleSheetBackgroundPlantingJob(value){
  if(!value || typeof value !== "object") return null;
  const eventId = getSafePositiveRecordId(value.eventId);
  ...
  return {
    ...
    nextAttemptAt: Math.max(0, Number(value.nextAttemptAt) || 0),
    successMessage: String(value.successMessage || "").slice(0, 500),
    failureMessage: String(value.failureMessage || "苗植え記録は端末内に保存されています。スプレッドシートは未送信です").slice(0, 500),
    showFailureDetails: value.showFailureDetails !== false
  };
}

function saveGoogleSheetBackgroundSendOutbox(){
  try{
    harvestnaviLocalStorage.writeJson(getActiveGoogleSheetSendOutboxStorageKey(), {
      version: 1,
      records: [...googleSheetBackgroundRecordQueue.entries()].map(([key, job]) => ({
        key,
        ...job
      })),
      plantingEvents: [...googleSheetBackgroundPlantingQueue.entries()].map(([key, job]) => ({
        key,
        ...job
      }))
    });
    ...
    console.error("Google Sheet送信待ちの保存に失敗しました", error);
    return false;
  }
}

function loadGoogleSheetBackgroundSendOutbox(){
  let stored = null;
  try{
    stored = harvestnaviLocalStorage.readJson(getActiveGoogleSheetSendOutboxStorageKey(), null);
  }catch(error){
    console.error("Google Sheet送信待ちを読み込めませんでした", error);
  }
  if(!stored || Number(stored.version) !== 1) return;

  (Array.isArray(stored.records) ? stored.records : []).forEach(value => {
    const job = normalizeGoogleSheetBackgroundRecordJob(value);
    const key = String(value?.key || "").trim()
      || (job ? getHarvestRecordIdentityKey(job) : "");
    if(job && key) googleSheetBackgroundRecordQueue.set(key, job);
  });
  (Array.isArray(stored.plantingEvents) ? stored.plantingEvents : []).forEach(value => {
    const job = normalizeGoogleSheetBackgroundPlantingJob(value);
    if(!job) return;
    googleSheetBackgroundPlantingQueue.set(String(job.eventId), job);
  });
}

function normalizeGoogleSheetAcceptedBatchEntry(value){
  if(!value || typeof value !== "object") return null;
  const batchId = String(value.batchId || "").trim();
  if(!/^[A-Za-z0-9_-]{1,80}$/.test(batchId)) return null;
  const recordSnapshots = Array.isArray(value.recordSnapshots)
    ? value.recordSnapshots.slice(0, GOOGLE_SHEET_MAX_BATCH_RECORDS)
    : [];
  const plantingEventSnapshots = Array.isArray(value.plantingEventSnapshots)
    ? value.plantingEventSnapshots.slice(0, GOOGLE_SHEET_MAX_BATCH_RECORDS)
    : [];
  if(!recordSnapshots.length && !plantingEventSnapshots.length) return null;
  return {
    batchId,
    ...
    checkCount: Math.max(0, Math.trunc(Number(value.checkCount) || 0)),
    nextCheckAt: Math.max(0, Number(value.nextCheckAt) || 0)
  };
}
...
function saveGoogleSheetAcceptedBatchQueue(options = {}){
  try{
    ...
    console.error("Google受信済み記録の確認情報を保存できませんでした", error);
    return false;
  }
}
...
function getGoogleSheetAcceptedBatchCheckDelay(checkCount){
  const delays = GOOGLE_SHEET_INBOX_STATUS_DELAYS_MS;
  const index = Math.min(Math.max(0, Math.trunc(Number(checkCount) || 0)), delays.length - 1);
  return delays[index];
}

function rememberGoogleSheetAcceptedDayBatch(
  batchId,
  recordSnapshots,
  ...
  const entry = normalizeGoogleSheetAcceptedBatchEntry({
    ...
    checkCount:0,
    nextCheckAt:Date.now() + getGoogleSheetAcceptedBatchCheckDelay(0)
  });
...
function getNextGoogleSheetAcceptedBatchCheckDelay(){
  if(!isGoogleSheetBackgroundNetworkAvailable() || !googleSheetAcceptedBatchQueue.size) return null;
  const now = Date.now();
  let delay = null;
  googleSheetAcceptedBatchQueue.forEach(entry => {
    const value = Math.max(0, (Number(entry.nextCheckAt) || 0) - now);
    delay = delay === null ? value : Math.min(delay, value);
  });
  return delay;
}

function scheduleGoogleSheetAcceptedBatchCheck(delay = 0){
  if(googleSheetAcceptedBatchCheckRunning) return;
  const queueDelay = getNextGoogleSheetAcceptedBatchCheckDelay();
  if(queueDelay === null) return;
  const effectiveDelay = Math.max(0, Number(delay) || 0, queueDelay);
  const dueAt = Date.now() + effectiveDelay;
  if(googleSheetAcceptedBatchCheckTimer !== null){
    if(googleSheetAcceptedBatchCheckTimerDueAt <= dueAt) return;
    clearTimeout(googleSheetAcceptedBatchCheckTimer);
  }
  googleSheetAcceptedBatchCheckTimerDueAt = dueAt;
  googleSheetAcceptedBatchCheckTimer = setTimeout(() => {
    googleSheetAcceptedBatchCheckTimer = null;
    googleSheetAcceptedBatchCheckTimerDueAt = 0;
    checkGoogleSheetAcceptedBatches().catch(error => {
      console.error("Google受信済み記録の確認に失敗しました", error);
    });
  }, effectiveDelay);
}

function markGoogleSheetAcceptedBatchFailed(entry, config){
  (entry?.recordSnapshots || []).forEach(snapshot => {
    const current = findHarvestRecordByIdentity(snapshot, records);
    if(!current) return;
    try{
      if(getGoogleSheetRecordSendSignature(current, config)
        === getGoogleSheetRecordSendSignature(snapshot, config)){
        setGoogleSheetSyncStatus(current, "failed");
      }
    }catch(error){
      console.warn("受信箱エラー後の収穫記録状態を更新できませんでした", error);
    }
  });
  (entry?.plantingEventSnapshots || []).forEach(snapshot => {
    const current = getPlantingEventById(snapshot?.eventId);
    if(!current) return;
    try{
      if(getPlantingEventSendSignature(current) === getPlantingEventSendSignature(snapshot)){
        setPlantingEventSyncStatus(current, "failed");
      }
    }catch(error){
      console.warn("受信箱エラー後の苗植え記録状態を更新できませんでした", error);
    }
  });
}

async function checkGoogleSheetAcceptedBatches(){
  if(googleSheetAcceptedBatchCheckRunning || !isGoogleSheetBackgroundNetworkAvailable()) return;
  const now = Date.now();
  const entry = [...googleSheetAcceptedBatchQueue.values()]
  ...
  if(!configValidation.ok || String(configValidation.config.url || "") !== entry.configUrl){
    entry.checkCount++;
  ...
  try{
    ...
    try{
      const response = await fetch(config.url, {
        ...
        body:buildValidatedGoogleSheetRequestBody(
          buildGoogleSheetDayBatchInboxStatusPayload(entry.batchId, config)
        ),
        signal:controller.signal
      });
      const text = await response.text();
      if(!isWithinGoogleSheetResponseLimits(text)) throw new Error("受信箱の応答が大きすぎます");
      result = text ? JSON.parse(text) : {};
      if(result.ok !== true) throw new Error(result.message || "受信箱の状態を確認できません");
    }finally{
      clearTimeout(timer);
    }
    googleSheetInboxSupportState = "supported";
    if(result.processed === true && result.queueStatus === "completed"){
      applyGoogleSheetAcceptedInboxResult(entry, result, config);
      acknowledgeGoogleSheetMutationRevision(
        config,
        entry.requestedSyncRevision,
        result
      );
      removeGoogleSheetAcceptedDayBatch(entry.batchId);
    }else if(result.failed === true || result.queueStatus === "failed"
    ...
    entry.nextCheckAt = Date.now() + getGoogleSheetAcceptedBatchCheckDelay(entry.checkCount);
    saveGoogleSheetAcceptedBatchQueue();
    console.warn("Google受信済み記録の状態確認を再試行します", error);
  }finally{
    googleSheetAcceptedBatchCheckRunning = false;
    updateRecordSyncStatusIndicator();
    scheduleGoogleSheetAcceptedBatchCheck();
  }
}
...
function updateRecordSyncStatusIndicator(summary = null){
  ...
  if(historyButton) historyButton.title = presentation.label;
}

function isGoogleSheetBackgroundNetworkAvailable(){
  return typeof navigator === "undefined" || navigator.onLine !== false;
}

function isGoogleSheetBackgroundRecordDeferred(job){
  const waitForPlantingRecordId = getSafePositiveRecordId(job?.waitForPlantingRecordId);
  if(waitForPlantingRecordId === null) return false;
  return recordSelectionMode === "planting"
    && workflowPlantingSessionActive
    && Number(activePlantingRecordId) === waitForPlantingRecordId;
}

function getGoogleSheetBackgroundJobWaitMs(job, now = Date.now()){
  return Math.max(0, (Number(job?.nextAttemptAt) || 0) - now);
}

function isGoogleSheetBackgroundRecordReady(job, now = Date.now()){
  return !isGoogleSheetBackgroundRecordDeferred(job)
    && getGoogleSheetBackgroundJobWaitMs(job, now) === 0;
}

function isGoogleSheetBackgroundPlantingReady(job, now = Date.now()){
  return getGoogleSheetBackgroundJobWaitMs(job, now) === 0;
}

function getNextGoogleSheetBackgroundSendDelay(){
  if(!isGoogleSheetBackgroundNetworkAvailable()) return null;
  const now = Date.now();
  let nextDelay = null;
  googleSheetBackgroundRecordQueue.forEach(job => {
    if(isGoogleSheetBackgroundRecordDeferred(job)) return;
    const delay = getGoogleSheetBackgroundJobWaitMs(job, now);
    nextDelay = nextDelay === null ? delay : Math.min(nextDelay, delay);
  });
  googleSheetBackgroundPlantingQueue.forEach(job => {
    const delay = getGoogleSheetBackgroundJobWaitMs(job, now);
    nextDelay = nextDelay === null ? delay : Math.min(nextDelay, delay);
  });
  return nextDelay;
...
function hasReadyGoogleSheetBackgroundRecordJob(){
  const now = Date.now();
  return [...googleSheetBackgroundRecordQueue.values()]
    .some(job => isGoogleSheetBackgroundRecordReady(job, now));
}

function hasReadyGoogleSheetBackgroundPlantingJob(){
  const now = Date.now();
  return [...googleSheetBackgroundPlantingQueue.values()]
    .some(job => isGoogleSheetBackgroundPlantingReady(job, now));
}

function scheduleGoogleSheetBackgroundJobRetry(job){
  const delays = getGoogleSheetBackgroundRetryDelays();
  const retryCount = Math.max(0, Math.trunc(Number(job?.retryCount) || 0));
  if(retryCount >= delays.length) return false;
  job.retryCount = retryCount + 1;
  job.nextAttemptAt = Date.now() + Math.max(0, Number(delays[retryCount]) || 0);
  return true;
}

function releaseGoogleSheetBackgroundRecordsForPlantingEvent(event){
  const sourceRecordIds = new Set((event?.sourceAllocations || [])
    .map(allocation => getSafePositiveRecordId(allocation?.harvestRecordId))
    .filter(id => id !== null));
  if(!sourceRecordIds.size) return false;
  let changed = false;
  googleSheetBackgroundRecordQueue.forEach(job => {
    const waitForPlantingRecordId = getSafePositiveRecordId(job?.waitForPlantingRecordId);
    if(waitForPlantingRecordId === null || !sourceRecordIds.has(waitForPlantingRecordId)) return;
    job.waitForPlantingRecordId = null;
    job.retryCount = 0;
    job.nextAttemptAt = 0;
    changed = true;
  });
...
function wakeGoogleSheetBackgroundSendQueue(){
  googleSheetBackgroundRecordQueue.forEach(job => { job.nextAttemptAt = 0; });
  googleSheetBackgroundPlantingQueue.forEach(job => { job.nextAttemptAt = 0; });
  saveGoogleSheetBackgroundSendOutbox();
  scheduleGoogleSheetBackgroundSend();
  scheduleGoogleSheetAcceptedBatchCheck();
}

function initializeGoogleSheetBackgroundSendQueue(){
  if(googleSheetBackgroundSendTimer !== null) clearTimeout(googleSheetBackgroundSendTimer);
  googleSheetBackgroundSendTimer = null;
  googleSheetBackgroundSendTimerDueAt = 0;
  googleSheetBackgroundRecordQueue.clear();
  googleSheetBackgroundPlantingQueue.clear();
  loadGoogleSheetBackgroundSendOutbox();
  loadGoogleSheetAcceptedBatchQueue();
  pruneConfirmedGoogleSheetBackgroundSendJobs();

  if(!googleSheetBackgroundSendEventsInstalled){
    window.addEventListener("online", wakeGoogleSheetBackgroundSendQueue);
    ...
    googleSheetBackgroundSendEventsInstalled = true;
  }
  scheduleGoogleSheetBackgroundSend();
  scheduleGoogleSheetAcceptedBatchCheck();
  updateRecordSyncStatusIndicator();
...
function reloadGoogleSheetBackgroundSendQueue(){
  initializeGoogleSheetBackgroundSendQueue();
}

function setGoogleSheetRecordBatchSyncStatus(recordsToUpdate, state, options = {}){
...
function scheduleGoogleSheetResendButtonStateUpdate(){
  if(scheduleGoogleSheetResendButtonStateUpdate.pending) return;
  scheduleGoogleSheetResendButtonStateUpdate.pending = true;
  setTimeout(() => {
    scheduleGoogleSheetResendButtonStateUpdate.pending = false;
    try{
      updateGoogleSheetResendButtonState();
    }catch(error){
      console.error("送信状態の表示を更新できませんでした", error);
    }
  }, 100);
}
...
function queueGoogleSheetRecordBatchSend(recordsToQueue, options = {}){
  ...
  try{
    setGoogleSheetRecordBatchSyncStatus(conflictedRecords, "conflict", { updateUi:false });
  }catch(error){
    console.error("収穫記録の送信状態を保存できませんでした", error);
    scheduleGoogleSheetResendButtonStateUpdate();
    return 0;
  }
  if(!queueableRecords.length){
    scheduleGoogleSheetResendButtonStateUpdate();
    return 0;
  }
  ...
  if(!validation.ok){
    try{
      setGoogleSheetRecordBatchSyncStatus(
        queueableRecords.map(item => item.record),
        "failed",
        { updateUi:false }
      );
    }catch(error){
      console.error("収穫記録の未送信状態を保存できませんでした", error);
    }
    scheduleGoogleSheetResendButtonStateUpdate();
    return 0;
  }
  ...
  const waitForPlantingRecordId = getSafePositiveRecordId(options.waitForPlantingRecordId);
  const previousJobs = new Map(queueableRecords.map(({ key }) => [
    key,
    googleSheetBackgroundRecordQueue.has(key)
      ? { ...googleSheetBackgroundRecordQueue.get(key) }
      : null
  ]));
  ...
  try{
    outboxSaved = saveGoogleSheetBackgroundSendOutbox();
  }catch(error){
    console.error("収穫記録の送信待ちを保存できませんでした", error);
  }
  if(!outboxSaved){
    previousJobs.forEach((job, key) => {
      if(job) googleSheetBackgroundRecordQueue.set(key, job);
      else googleSheetBackgroundRecordQueue.delete(key);
    });
    try{
      setGoogleSheetRecordBatchSyncStatus(
        queueableRecords.map(item => item.record),
  ...
  scheduleGoogleSheetResendButtonStateUpdate();
  // 完了表示を先に描画できるよう、通信開始は入力操作のフレームから離す。
  scheduleGoogleSheetBackgroundSend(100);
...
function queueGoogleSheetPlantingEventSend(event, options = {}){
  ...
  if(hasSyncConflictForEntity("planting", event)){
    try{
      setPlantingEventSyncStatus(event, "conflict", { updateUi:false });
    }catch(error){
      console.error("苗植え記録の競合状態を保存できませんでした", error);
    }
    scheduleGoogleSheetResendButtonStateUpdate();
    return false;
  }
  ...
  if(!validation.ok){
    try{
      setPlantingEventSyncStatus(event, "failed", { updateUi:false });
    }catch(error){
      console.error("苗植え記録の未送信状態を保存できませんでした", error);
    }
    scheduleGoogleSheetResendButtonStateUpdate();
    return false;
  }
  ...
  const previousRecordJobs = new Map([...googleSheetBackgroundRecordQueue.entries()]
    .map(([key, value]) => [key, { ...value }]));
  const previousPlantingJob = googleSheetBackgroundPlantingQueue.has(String(eventId))
    ? { ...googleSheetBackgroundPlantingQueue.get(String(eventId)) }
    : null;
  try{
    setPlantingEventSyncStatus(event, "edited", { updateUi:false });
  }catch(error){
    console.error("苗植え記録の送信状態を保存できませんでした", error);
    scheduleGoogleSheetResendButtonStateUpdate();
    return false;
  }
  releaseGoogleSheetBackgroundRecordsForPlantingEvent(event);
  googleSheetBackgroundPlantingQueue.set(String(eventId), job);
  let outboxSaved = false;
  try{
    outboxSaved = saveGoogleSheetBackgroundSendOutbox();
  }catch(error){
    console.error("苗植え記録の送信待ちを保存できませんでした", error);
  }
  if(!outboxSaved){
    googleSheetBackgroundRecordQueue.clear();
    previousRecordJobs.forEach((value, key) => googleSheetBackgroundRecordQueue.set(key, value));
    if(previousPlantingJob){
      googleSheetBackgroundPlantingQueue.set(String(eventId), previousPlantingJob);
    }else{
      googleSheetBackgroundPlantingQueue.delete(String(eventId));
    }
    try{
      setPlantingEventSyncStatus(event, "failed", { updateUi:false });
    }catch(error){
...
function scheduleGoogleSheetBackgroundSend(delay = 0){
  ...
  const queueDelay = getNextGoogleSheetBackgroundSendDelay();
  if(queueDelay === null) return;
  const effectiveDelay = Math.max(0, Number(delay) || 0, queueDelay);
  const dueAt = Date.now() + effectiveDelay;
  if(googleSheetBackgroundSendTimer !== null){
    if(googleSheetBackgroundSendTimerDueAt <= dueAt) return;
    clearTimeout(googleSheetBackgroundSendTimer);
  }
  googleSheetBackgroundSendTimerDueAt = dueAt;
  ...
  for(const [key, job] of googleSheetBackgroundRecordQueue){
    ...
    if(!record){
      if(googleSheetBackgroundRecordQueue.get(key) === job){
        ...
        removedMissingJob = true;
      }
      continue;
    }
    batch.push({ key, job, record });
  }
  if(removedMissingJob) saveGoogleSheetBackgroundSendOutbox();
  return batch;
}
...
function collectGoogleSheetBackgroundDayBatch(){
  ...
  googleSheetBackgroundRecordQueue.forEach((job, key) => {
    if(!isGoogleSheetBackgroundRecordReady(job, now)) return;
    const record = findGoogleSheetBackgroundRecord(job);
    if(!record){
      if(googleSheetBackgroundRecordQueue.get(key) === job){
        googleSheetBackgroundRecordQueue.delete(key);
  ...
  googleSheetBackgroundPlantingQueue.forEach((job, key) => {
    if(!isGoogleSheetBackgroundPlantingReady(job, now)) return;
...
function pruneConfirmedGoogleSheetBackgroundSendJobs(){
  ...
  googleSheetBackgroundRecordQueue.forEach((job, key) => {
    const record = findGoogleSheetBackgroundRecord(job);
    if(!record || !["confirmed", "accepted"].includes(getGoogleSheetRecordSyncState(record))) return;
    if(googleSheetBackgroundRecordQueue.get(key) === job){
      googleSheetBackgroundRecordQueue.delete(key);
      changed = true;
    }
  ...
  googleSheetBackgroundPlantingQueue.forEach((job, key) => {
    const event = getPlantingEventById(job.eventId);
    const eventState = String(plantingStatus[String(job.eventId)]?.state || "");
    if(!event || (isPlantingEventUnsent(event, plantingStatus) && eventState !== "accepted")) return;
    if(googleSheetBackgroundPlantingQueue.get(key) === job){
      googleSheetBackgroundPlantingQueue.delete(key);
      changed = true;
    }
  });
  if(changed) saveGoogleSheetBackgroundSendOutbox();

  let acceptedChanged = false;
  const recordStatus = loadGoogleSheetSyncStatus();
  const latestPlantingStatus = loadPlantingEventSyncStatus();
  googleSheetAcceptedBatchQueue.forEach((entry, batchId) => {
    const recordsConfirmed = (entry.recordSnapshots || []).every(snapshot => {
      const current = findHarvestRecordByIdentity(snapshot, records);
      return current && getGoogleSheetRecordSyncState(current, recordStatus) === "confirmed";
    });
    const plantingConfirmed = (entry.plantingEventSnapshots || []).every(snapshot => {
      const eventId = getSafePositiveRecordId(snapshot?.eventId);
      return eventId !== null
        && String(latestPlantingStatus[String(eventId)]?.state || "") === "confirmed";
    });
    if(recordsConfirmed && plantingConfirmed){
      googleSheetAcceptedBatchQueue.delete(batchId);
...
async function sendGoogleSheetBackgroundRecordBatch(){
  ...
  try{
    ...
    try{
      result = await sendRecordsBatchToGoogleSheet(
        batch.map(item => item.record),
        { showFailureDialog: false, showConfigNotice: false }
      );
      retryableFailure = result?.retryableFailure === true;
    }catch(error){
      ...
      retryableFailure = isRetryableGoogleSheetSendError(error);
    ...
    batch.forEach(({ key, job }) => {
      ...
      if(googleSheetBackgroundRecordQueue.get(key) !== job) return;
      const currentRecord = findGoogleSheetBackgroundRecord(job);
      if(currentRecord && ["confirmed", "accepted"].includes(getGoogleSheetRecordSyncState(currentRecord))){
        googleSheetBackgroundRecordQueue.delete(key);
      }else if(retryableFailure && scheduleGoogleSheetBackgroundJobRetry(job)){
        retryScheduled = true;
      }else{
        googleSheetBackgroundRecordQueue.delete(key);
        if(job.failureMessage && !failureMessage) failureMessage = job.failureMessage;
      }
    });
    saveGoogleSheetBackgroundSendOutbox();
...
function isRetryableGoogleSheetSendError(error){
...
function getGoogleSheetBackgroundResultRetryableFailure(result, error){
  return result?.retryableFailure === true || isRetryableGoogleSheetSendError(error);
}
...
async function sendGoogleSheetBackgroundDayBatch(){
  ...
  try{
    ...
    const retryableFailure = getGoogleSheetBackgroundResultRetryableFailure(result, sendError);
    let failureMessage = "";
    let retryScheduled = false;
    ...
    batch.plantingEvents.forEach(({ key, job }) => {
      ...
      const plantingStatus = loadPlantingEventSyncStatus();
      const currentEventState = String(plantingStatus[String(job.eventId)]?.state || "");
      if(currentEvent && (!isPlantingEventUnsent(currentEvent, plantingStatus)
        || currentEventState === "accepted")){
        googleSheetBackgroundPlantingQueue.delete(key);
      }else if(retryableFailure && scheduleGoogleSheetBackgroundJobRetry(job)){
        retryScheduled = true;
      }else{
...
async function runGoogleSheetBackgroundSendQueue(){
  ...
  try{
    while(googleSheetSendState === "idle" && !googleSheetOperationOwner){
      if(hasReadyGoogleSheetBackgroundPlantingJob() || hasReadyGoogleSheetBackgroundRecordJob()){
...
function createGoogleSheetDayBatchId(recordSnapshots = [], plantingEventSnapshots = [], config = {}){
  const source = JSON.stringify({
    records:(Array.isArray(recordSnapshots) ? recordSnapshots : [])
      .map(record => getGoogleSheetRecordSendSignature(record, config)),
    plantingEvents:(Array.isArray(plantingEventSnapshots) ? plantingEventSnapshots : [])
      .map(getPlantingEventSendSignature)
  });
  ...
  return `day-${source.length.toString(36)}-${first.toString(36)}-${second.toString(36)}`;
}

function buildGoogleSheetDayBatchPayload(recordsToSend, plantingEventsToSend, config, options = {}){
...
function buildGoogleSheetDayBatchInboxPayload(recordsToSend, plantingEventsToSend, config, options = {}){
  const payload = buildGoogleSheetDayBatchPayload(
    recordsToSend,
    plantingEventsToSend,
    config,
    options
  );
  return {
    ...payload,
    type:"harvest-day-batch-inbox",
    action:"enqueueDayBatch"
  };
}

function buildGoogleSheetDayBatchInboxStatusPayload(batchId, config){
  return {
    app:"Harvestnavi",
...
function buildGoogleSheetDayBatchStatusPayload(recordsToConfirm, plantingEventsToConfirm, config, options = {}){
  const batchId = String(options.batchId || "").trim();
  return {
    app: "Harvestnavi",
...
function getPlantingEventForGoogleTransfer(event){
  ...
  return {
    ...
    seedlingHousePrimaryPlantingDate: normalized.seedlingHousePrimaryPlantingDate,
    seedlingHouseNextStartKey: normalized.seedlingHouseNextStartKey,
